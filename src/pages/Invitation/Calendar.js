  ...
  return (
    <div className="rounded-2xl border border-rose-100 bg-rose-50 p-4">
      <div className="mb-2 flex items-center justify-between">
        <button onClick={prevMonth} className="h-8 w-8 rounded-full hover:bg-white/70" aria-label="이전 달">‹</button>
        <div className="text-sm font-medium tracking-wide text-rose-700">
          {view.getFullYear()}년 {view.getMonth() + 1}월
        </div>
        <button onClick={nextMonth} className="h-8 w-8 rounded-full hover:bg-white/70" aria-label="다음 달">›</button>
      </div>
      ...
      <div className="grid grid-cols-7 gap-y-1 text-center">
        {cells.map((d, i) => {
          ...
          const base = "mx-auto my-1 flex h-9 w-9 items-center justify-center rounded-full text-sm";
