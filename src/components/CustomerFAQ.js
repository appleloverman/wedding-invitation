...
export const askedQuestions = [
  ...
  },
  {
    ...
    answer:
      "마이페이지 > 주문내역에서 교환 신청이 가능합니다. 또는 고객센터로 문의해주세요.",
  },
  {
    id: 15,
    date: "2025-08-30",
  ...
  },
  {
    ...
    answer:
      "불편을 드려 죄송합니다. 고객센터로 사진과 함께 문의 주시면 빠르게 처리해드리겠습니다.",
  },
  {
    id: 23,
    date: "2025-08-30",
...
const CustomerFAQ = () => {
  const [searchQuery, setSearchQuery] = useState(""); // 검색용 스테이트 input란에 onChange 적용
  const [selectedQuestion, setSelectedQuestion] = useState(null); // 질문검색용 스테이트

  const filteredQuestions = askedQuestions.filter((i) =>
    i.comment.includes(searchQuery)
  );

  // 같으면 닫고, 틀리면 열다
  const handleQuestionClick = (i) => {
    setSelectedQuestion(selectedQuestion === i.id ? null : i.id);
  };
        ...
        {filteredQuestions.length === 0 ? (
          ...
          filteredQuestions.map((question) => (
            <div key={question.id} className="faq-item">
              ...
              >
                <span className="faq-q-icon">Q</span> {question.comment}
